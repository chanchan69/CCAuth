...
aid = 'a36kVacX2uybml8pFPAn'
client_secret = 'QCU0CZHF3QnvueKDq-gStEk-cA2ma-SX6fVhw6B7Ru8='
...
def authenticate(username, password, hwid):
    ...
    s = requests.get(url=f'http://sirchanchan21.pythonanywhere.com/api/v2/authenticate?key={apikey}', headers={"data": payload, "aid": aid}).text.encode()
    #This one for no hash ^
    #s = requests.get(url=f'http://sirchanchan21.pythonanywhere.com/api/v2/authenticate?key={apikey}', headers={"data": payload, "aid": aid, "hash": Get_Hash()}).text.encode()
    #This one for hash ^
